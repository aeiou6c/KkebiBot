...
namespace KkebiBot.CommandModules
{
    public class ExampleUngrouppedCommands : BaseCommandModule
    {
        ...
        [Command("야")]
        public async Task TestCommand_1(CommandContext ctx) // this command takes no arguments
        {
            // let's trigger a typing indicator to let
            // users know we're working
            await ctx.TriggerTypingAsync();
        ...
        [Command("뭐")]
        public async Task TestCommand_2(CommandContext ctx) // this command takes no arguments
        {
            // let's trigger a typing indicator to let
            // users know we're working
            await ctx.TriggerTypingAsync();
    ...
    .SendAsync(ctx.Channel);
        ...
        [Command("주사위")]
        public async Task TestCommand_3(CommandContext ctx) // this command takes no arguments
        {
            // let's trigger a typing indicator to let
            // users know we're working
            await ctx.TriggerTypingAsync();
            ...
            await ctx.RespondAsync($"Your number is: {random.Next(1, 6)}");
        }
